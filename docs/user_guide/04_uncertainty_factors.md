...
A task that omits the `uncertainty_factors` block entirely still receives these defaults, resulting in a combined multiplier based on the default levels. Each default level maps to a multiplier of exactly `1.0` in the default configuration, so the combined multiplier for a task with no explicit uncertainty factors is:
