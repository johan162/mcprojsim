...
This chapter explains how `mcprojsim` models schedule variation due to **risks**. (The other schedule variation comes from **uncertainty factors** See [uncertainty_factors](uncertainty_factors.md)). Both influence the simulated project duration, but they represent fundamentally different ideas and are handled differently by the simulation engine.
